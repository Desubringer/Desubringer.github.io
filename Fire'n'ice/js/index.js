		...
		if (this.frame < this.frames || this.state == 'burn') {
			{
				this.frame++;
				keypressed = null;
			} //current animation sequence continued, input flushed (user-friendly safety measure)
			...
			switch (this.state) { //ANIMATIONS AFTERMATH START:
				...
				case 'jump':
					field[this.row][this.column] = 'empty';
					this.row--;
					this.dir ? this.column++ : this.column--;
					if (field[this.row][this.column] == 'fire' ||
						field[this.row + 1][this.column] == 'JAR' ||
						field[this.row + 1][this.column] == '+JAR' ||
						field[this.row + 1][this.column] == '+JAR+' ||
		...
		if (!this.duck) //STANDING TALL SPRITES
			switch (this.state) {
				case 'turn':
					...
					drawSquare(this.row, this.column);
					drawSquare(this.row, this.column - (this.dir ? 1 : -1));
					ctx.drawImage(this.image, 201 + (this.frame * 48), this.dir ? 76 : 12, 34, 52, this.column * blockSize, (this.row - 0.5) * blockSize, blockSize, blockSize * 1.5);
					break;
				case 'run':
					for (let i = -1; i < 2; i++) {
				...
				case 'push':
					drawSquare(this.row - 1, this.column);
					drawSquare(this.row, this.column);
					ctx.drawImage(this.image, 584, this.dir ? 12 : 76, 32, 52, this.column * blockSize, (this.row - 0.5) * blockSize, blockSize, blockSize * 1.5);
					break;
				case 'burn':
					ctx.drawImage(this.image, 1112 + (this.frame % 2 * 48), 0, 34, 64, this.column * blockSize, (this.row - 0.5) * blockSize, blockSize, blockSize * 1.5);
					break;
				case 'spell':
					drawSquare(this.row - 1, this.column);
					drawSquare(this.row, this.column);
				...
				case 'jump':
					...
					ctx.drawImage(this.image, 584 + (this.frame && 48) + (this.frame > 2 && 48) + (this.frame > 3 && 48), this.dir ? 8 : 72, 32, 54,
						(this.column + (this.frame > 2 && (this.dir ? 0.2 : -0.2)) + (this.frame > 3 && (this.dir ? 0.4 : -0.4))) * blockSize, (this.row - 0.5 - (this.frame > 2 && 0.4) - (this.frame > 3 && 0.3)) * blockSize, blockSize, blockSize * 1.5);
					break;
					break;
				case 'fall':
					drawSquare(this.row - 1, this.column);
					...
					break;
				default:
					throw ('Dana ain\'t well today. Must\'ve caught FIRE MWAHAHAHAHAHA');
			}
		else //CROUCHING SPRITES, base offset: 968
			switch (this.state) {
				case 'turn':
					drawSquare(this.row, this.column);
					ctx.drawImage(this.image, 968, this.dir ? 76 : 12, 34, 52, this.column * blockSize, (this.row - 0.5) * blockSize, blockSize, blockSize * 1.5);
					break;
				case 'run':
					drawSquare(this.row, this.column);
					drawSquare(this.row - 1, this.column);
				...
				case 'push':
					drawSquare(this.row, this.column);
					ctx.drawImage(this.image, 1065, this.dir ? 12 : 76, 34, 52, this.column * blockSize, (this.row - 0.5) * blockSize, blockSize, blockSize * 1.5);
					break;
				case 'burn':
					ctx.drawImage(this.image, 1112 + (this.frame % 2 * 48), 76, 34, 52, this.column * blockSize, (this.row - 0.5) * blockSize, blockSize, blockSize * 1.5);
					break;
				case 'spell':
					drawSquare(this.row, this.column);
					drawSquare(this.row, this.column + (this.dir ? 1 : -1));
					...
					break;
				default:
					throw ('Dana ain\'t well today. Must be evil sorcery');
			}
	} //Player.Draw()
		...
		for (let column = 0; column < field[row].length; column++) {
			if (!blockStart) {
				switch (field[row][column]) {
					case 'fire':
						let underFire = field[row + 1][column];
					...
					case 'JAR+':
						...
						break;
					case 'ice':
						if (field[row + 1][column] == 'empty' || field[row + 1][column] == 'fire') {
							fallMustGoOn = {
								colStart: column,
								colEnd: column,
								row: row,
						...
						break;
					case 'metal':
						if (field[row + 1][column] == 'empty' || field[row][column] == 'fire') {
							fallMustGoOn = {
								colStart: column,
								colEnd: column,
								row: row,
					...
					case 'metal+':
						...
						break;
					default:
						; //others, we just skip, for they are but FALLESS. Or, well, bugged?.
				}
				...
				if (field[row + 1][column] != 'empty' && field[row + 1][column] != 'fire') { //the ice-welded block may continue, but it just isn't going to fall
					...
					switch (field[row][column]) { //otherwise, have we reached the right end?
						...
						case '+ice':
							...
							return;
						default:
							1; //console.log('the search is now on ' + row + ' row,' + column + ' column, and goes on');
					}
				}
			}
		}
		...
		for (let i = colStart; i <= colEnd; i++) {
			...
			switch (field[row][i]) {
				case 'ice':
					...
					break;
				case 'ice+':
					ctx.drawImage(mapObjects, 16, 160, 16.1, 16, i * blockSize, (row + (15 - fallMustGoOn.frames) / 15) * blockSize, blockSize, blockSize);
					break;
				case '+ice+':
					ctx.drawImage(mapObjects, 32, 160, 16, 16, i * blockSize, (row + (15 - fallMustGoOn.frames) / 15) * blockSize, blockSize, blockSize);
					break;
				case '+ice':
					ctx.drawImage(mapObjects, 47.9, 160, 16, 16, i * blockSize, (row + (15 - fallMustGoOn.frames) / 15) * blockSize, blockSize, blockSize);
					break;
				case 'metal':
					ctx.drawImage(mapObjects, 63, 160, 16, 16, i * blockSize, (row + (15 - fallMustGoOn.frames) / 15) * blockSize, blockSize, blockSize);
					break;
				case '+metal':
					ctx.drawImage(mapObjects, 80, 160, 8, 16, i * blockSize, (row + (15 - fallMustGoOn.frames) / 15) * blockSize, blockSize / 2, blockSize);
					ctx.drawImage(mapObjects, 71, 160, 8, 16, (i + 0.5) * blockSize, (row + (15 - fallMustGoOn.frames) / 15) * blockSize, blockSize / 2, blockSize);
					break;
				case 'metal+':
					ctx.drawImage(mapObjects, 63, 160, 8, 16, i * blockSize, (row + (15 - fallMustGoOn.frames) / 15) * blockSize, blockSize / 2, blockSize);
				...
				case 'fire':
					...
					break;
				default:
					console.log('is this even supposed to fall? i mean, ' + field[row][i] + ' from row ' + row + ' col ' + i);
			}
		}
	} else { //smth fault
			...
			if (below[i - colStart] == 'fire') { //the most interesting part: if there was a fire below
				...
				switch (field[row][i]) { //now. we iterate through the chain left to right, so [row]'n'[col] of a mutating square are bound to differ, watch out!
					case 'ice':
						...
						break;
					case '+ice':
						switch (field[row + 1][i - 1]) { //right edge melts, only previous block affected
							case 'ice+':
								field[row + 1][i - 1] = 'ice';
								break;
							case '+ice+':
								...
								break;
							case 'metal+':
								field[row + 1][i - 1] = 'metal';
								break;
							case '+metal+':
								field[row + 1][i - 1] = '+metal';
								break;
							default:
								console.log('N-n-nani?');
						}
						field[row + 1][i] = 'empty';
						break;
					case '+ice+':
						switch (field[row + 1][i - 1]) { //middle block melts, prev block affected
							case 'ice+':
								field[row + 1][i - 1] = 'ice';
								break;
							case '+ice+':
						...
						break;
					case 'ice+':
						switch (field[row][i + 1]) { //left edge melts, only next block affected
							case '+ice':
								field[row][i + 1] = 'ice';
								break;
							case '+ice+':
						...
						break;
					default:
						field[row + 1][i] = field[row][i]; //this fires for every immeltable (~metal) piece
				}
...
function drawSquare(row, column) {
	...
	switch (field[row][column]) {
		...
		case '+ground+':
			ctx.drawImage(mapObjects, 112, (lvl - 1) * 16, 16, 16, column * blockSize, row * blockSize, blockSize, blockSize);
			break;
		case 'ground+':
		...
		case 'ice':
			ctx.drawImage(mapObjects, 0, engine.frame % 5 ? 176 : 160, 16, 16, column * blockSize, row * blockSize, blockSize, blockSize);
			break;
		case 'ice+':
		...
		case '+ice':
			...
			break;
		case 'metal':
			ctx.drawImage(mapObjects, 63, engine.frame % 5 ? 176 : 160, 16, 16, column * blockSize, row * blockSize, blockSize, blockSize);
			break;
		case '+metal':
			ctx.drawImage(mapObjects, 80, engine.frame % 5 ? 176 : 160, 8, 16, column * blockSize, row * blockSize, blockSize / 2, blockSize);
			ctx.drawImage(mapObjects, 71, engine.frame % 5 ? 176 : 160, 8, 16, (column + 0.5) * blockSize, row * blockSize, blockSize / 2, blockSize);
			break;
		case 'metal+':
			ctx.drawImage(mapObjects, 63, engine.frame % 5 ? 176 : 160, 8, 16, column * blockSize, row * blockSize, blockSize / 2, blockSize);
			ctx.drawImage(mapObjects, 88, engine.frame % 5 ? 176 : 160, 8, 16, (column + 0.5) * blockSize, row * blockSize, blockSize / 2, blockSize);
			break;
		case '+metal+':
			ctx.drawImage(mapObjects, 80, engine.frame % 5 ? 176 : 160, 16, 16, column * blockSize, row * blockSize, blockSize, blockSize);
			break;
		case 'jar':
			ctx.drawImage(mapObjects, 128, 0, 16, 16, column * blockSize, row * blockSize, blockSize, blockSize);
			break;
		case '+jar':
			ctx.drawImage(mapObjects, 144, 0, 8, 16, column * blockSize, row * blockSize, blockSize / 2, blockSize);
			ctx.drawImage(mapObjects, 136, 0, 8, 16, (column + 0.5) * blockSize, row * blockSize, blockSize / 2, blockSize);
			break;
		case '+jar+':
			ctx.drawImage(mapObjects, 144, 0, 16, 16, column * blockSize, row * blockSize, blockSize, blockSize);
			break;
		case 'jar+':
			ctx.drawImage(mapObjects, 128, 0, 8, 16, column * blockSize, row * blockSize, blockSize / 2, blockSize);
			ctx.drawImage(mapObjects, 152, 0, 8, 16, (column + 0.5) * blockSize, row * blockSize, blockSize / 2, blockSize);
			break;
		case 'JAR':
			ctx.drawImage(mapObjects, 128, 16 * (1 + engine.frame), 16, 16, column * blockSize, row * blockSize, blockSize, blockSize);
			break;
		case '+JAR':
			ctx.drawImage(mapObjects, 144, 16 * (1 + engine.frame), 8, 16, column * blockSize, row * blockSize, blockSize / 2, blockSize);
			ctx.drawImage(mapObjects, 136, 16 * (1 + engine.frame), 8, 16, (column + 0.5) * blockSize, row * blockSize, blockSize / 2, blockSize);
			break;
		case '+JAR+':
			ctx.drawImage(mapObjects, 144, 16 * (1 + engine.frame), 16, 16, column * blockSize, row * blockSize, blockSize, blockSize);
			break;
		case 'JAR+':
			ctx.drawImage(mapObjects, 128, 16 * (1 + engine.frame), 8, 16, column * blockSize, row * blockSize, blockSize / 2, blockSize);
...
function slide() {
	...
	if (slideMustGoOn.framesLeft) { //our little pretty slide smoothering 'while' substitute
		...
		switch (onTheWay) { //now let's check what we have bumped into, hence induce proper outcomes
			case 'empty':
				if (below == 'empty' || below == 'fire') { //check whether this single block can(ergo must) fall now 
					fallMustGoOn = {
						row: row,
						colStart: column,
						colEnd: column,
				...
				if (field[row][column + delta] == 'empty' || field[row][column + delta] == 'fire') { //feels slidey still?
					if (isMetal && below != 'ice' &&
						...
						fieldUpdate(row);
						return;
					} //if metal, and no ice below it -> halt, re-animate, check gravity
					slideMustGoOn = {
						delta: delta,
						row: row,
						column: column,
						framesLeft: 15,
						isMetal: isMetal
					}; //else go on sliding, parameters renewed
					field[row][column] = 'empty';
				} else {
					fieldUpdate(row);
					engine.squaresToAnimate.push([row, column]);
				} //re-animate the block that reached the dead end
				break;
			case 'fire':
				extinguish(row, column); //first, we're, like, pshhh
				if (!isMetal) {
					field[row][column] = 'empty';
					drawSquare(row, column);
...
function iceMagic(row, column) { //this function mostly relies on working with strings
	...
	switch (field[row][column]) {
		case 'empty':
			if (field[row + 1][column] == 'JAR' || field[row + 1][column] == '+JAR' ||
